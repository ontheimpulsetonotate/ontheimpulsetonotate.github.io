...
export default class TruncateMarkup extends React.Component {
  ...
  static propTypes = {
    ...
    tokenize: (props, propName, componentName) => {
      ...
      if (
        typeof tokenizeValue !== 'undefined' &&
        !TOKENIZE_POLICY[tokenizeValue]
      ) return new Error(
  ...
  componentDidUpdate() {
    ...
    console.log('endFound', this.endFound)
    if (this.endFound) {
      ...
      if (
        ...
        console.log('textChanged', this.latestThatFits)
        this.setState({
  ...
  tryToFit(rootEl, splitDirections) {
    ...
    if (!rootEl.props.children) return
    console.log('tryToFit', [...splitDirections])
  ...
  splitString(string, splitDirections = [], level) {
    ...
    if (splitDirections.length && this.policy.isAtomic(string)) {
      ...
      if (!this.wasLastCharTested) this.wasLastCharTested = true

      // we are trying to split further but we have nowhere to go now
      // that means we've already found the max subtree that fits the container
      else this.endFound = true
  ...
  splitArray(array, splitDirections = [], level) {
    if (!splitDirections.length) return array

    if (array.length === 1) return [this.split(array[0], splitDirections, /* isRoot */ false, level)]
